...
export default function AdminEventsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const slug = editingId ? slugBase : `${slugBase}-${Date.now()}`
      ...
      if (editingId) {
        ...
        if (error) {
          console.error('Update error raw:', JSON.stringify(error, null, 2))
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    <button
                      type="button"
                      onClick={() =>
                        copyText(event.instagram_caption, 'Instagram caption')
                      }
                      className="rounded-full border border-white/15 px-4 py-2 text-sm text-white/80 transition hover:bg-white/10"
                    >
                      Copy Instagram
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        copyText(event.facebook_caption, 'Facebook caption')
                      }
                      className="rounded-full border border-white/15 px-4 py-2 text-sm text-white/80 transition hover:bg-white/10"
                    >
                      Copy Facebook
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        copyText(event.google_short_text, 'Google text')
                      }
                      className="rounded-full border border-white/15 px-4 py-2 text-sm text-white/80 transition hover:bg-white/10"
                    >
                      Copy Google
